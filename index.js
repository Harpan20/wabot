...
const startSock = async () => {
    ...
    sock.ev.on('messages.upsert', async m => {
        ...
        if (!msg.key.fromMe && m.type === 'notify') {
            if (msg.message) {
                if (msg.message.conversation.includes('#semarak jatimekar') && !msg.message.conversation.includes('Berhasil dimasukkan')) {
                    ...
                    if (layanan.length > 0){
                        ...
                        await sock.sendMessage(msg.key.remoteJid, {
                            text: "Sampurasun! ini merupakan bot layanan Masyarakat Desa Jatimekar"
                        });
                    ...
                    await sock.sendMessage(msg.key.remoteJid, {
                        text: "_Apa bila ingin membatalkan kirim pesan *Batal*_"
                    });
                    ...
                    try {
                        ...
                        await sock.sendMessage(msg.key.remoteJid, {
                            text: `NIK yang anda masukkan salah atau tidak terdaftar, silahkan masukkan ulang NIK`
                        });
                    }

                    if (masyarakat.length > 0){
                        await sock.sendMessage(msg.key.remoteJid, {
                            text: `NIK yang anda masukkan terdaftar`
                        }); 
// Menambahakan Tabel identitas
                        await sock.sendMessage(msg.key.remoteJid, {
                            text: `NIK: ${masyarakat[0].nik}\nNAMA:${masyarakat[0].nama}\nAlamat: ${masyarakat[0].alamat}\nJenis Kelamin: ${masyarakat[0].jenis_kelamin}\nAgama: ${masyarakat[0].agama}\nPekerjaan: ${masyarakat[0].pekerjaan}\nStatus Perkawinan: ${masyarakat[0].status_perkawinan}`
                        }); 

                        data.masyarakat = masyarakat[0]

                        let string_persyaratan_layanan = "Untuk memulai pembuatan surat silakan ketik :"
...
Jenis Kelamin : ${data.masyarakat.jenis_kelamin}\n
...
Apakah semua data diatas sudah benar ? (Ya/Tidak)`;
                    ...
                    if(msg.message.conversation.toLowerCase().includes('tidak')){
                        ...
                        try {
                            ...
                            await sock.sendMessage(msg.key.remoteJid, {
                                text: `Permintaan Berhasil ditambahkan, surat keterangan sedang dibuat mohon tunggu...`
                            });
                        } catch (error) {
                            await sock.sendMessage(msg.key.remoteJid, {
                    ...
                    await sock.sendMessage(msg.key.remoteJid, {
                        text: `Harap gunakan #semarak jatimekar untuk memulai dan menggunakan bot`
                    });
                }
            }
        }
